// PQ (Perceptual Quantizer) Transfer Function Module
// Based on SMPTE ST 2084:2014 (also in ITU-R BT.2100)

/**
 * PQ transfer function implementation
 * Reference: https://en.wikipedia.org/wiki/Perceptual_quantizer
 * 
 * PQ is designed to encode absolute luminance from 0 to 10,000 cd/m² (nits)
 */
export const PQ = {
     ...
     * 
     * Formula: E' = ((c1 + c2 * Y^m1) / (1 + c3 * Y^m1))^m2
     * where Y = luminance / 10000
     */
    encode: (linear) => {
     ...
     * 
     * Formula: Y = ((max(E'^(1/m2) - c1, 0)) / (c2 - c3 * E'^(1/m2)))^(1/m1)
     * Returns luminance in cd/m² (nits)
     */
    decode: (pq) => {
        const { m1, m2, c1, c2, c3, peakNits } = PQ.constants;
